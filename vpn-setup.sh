...
# OS:       Ubuntu/Debian, RHEL/CentOS/Rocky/Alma/Oracle, Fedora, openSUSE/SLES
#==============================================================================
...
# VPN IP Ranges
IKEV2_SERVER_IP="172.22.16.1"
IKEV2_SUBNET="172.22.16.0/20"
IKEV2_POOL_START="172.22.16.10"
IKEV2_POOL_END="172.22.31.250"
IKEV2_POOL="${IKEV2_POOL_START}-${IKEV2_POOL_END}"
IKEV2_SUBNET6="fdab:82a1:6c44:10::/64"
IKEV2_CERT_SUBNET="172.22.32.0/20"
IKEV2_CERT_POOL_START="172.22.32.10"
IKEV2_CERT_POOL_END="172.22.47.250"
IKEV2_CERT_POOL="${IKEV2_CERT_POOL_START}-${IKEV2_CERT_POOL_END}"
IKEV2_CERT_SUBNET6="fdab:82a1:6c44:20::/64"

L2TP_SERVER_IP="172.22.48.1"
L2TP_SUBNET="172.22.48.0/20"
L2TP_POOL_START="172.22.48.10"
L2TP_POOL_END="172.22.63.250"
L2TP_POOL="${L2TP_POOL_START}-${L2TP_POOL_END}"

WG_SERVER_IP="172.22.64.1"
WG_SUBNET="172.22.64.0/20"
WG_FIRST_CLIENT="172.22.64.2"
WG_POOL_END="172.22.79.254"
WG_SUBNET6="fdab:82a1:6c44:30::/64"
WG_SERVER_IP6="fdab:82a1:6c44:30::1"

OVPN_SERVER_IP="172.22.80.1"
OVPN_SUBNET="172.22.80.0/20"
OVPN_FIRST_CLIENT="172.22.80.2"
OVPN_POOL_END="172.22.95.254"
OVPN_SUBNET6="fdab:82a1:6c44:40::/64"
...
# IKEv2 MSS clamp for NAT-T clients
IKEV2_TCPMSS_MIN="1361"
IKEV2_TCPMSS_MAX="1536"
IKEV2_TCPMSS_VALUE="1360"
...
# Escape a string for embedding in XML text content or attribute values
# Escapes: & → &amp;  < → &lt;  > → &gt;  " → &quot;  ' → &apos;
xml_escape() {
    local val="$1"
    val="${val//&/&amp;}"
...
# Escapes: \ → \\  " → \"  and control characters (newline, tab, CR)
json_escape() {
    local val="$1"
    val="${val//\\/\\\\}"
    val="${val//\"/\\\"}"
...
# Escapes: ` → ``  " → `"  $ → `$  (backtick is the PS escape character)
ps_escape() {
    local val="$1"
    val="${val//\`/\`\`}"
    val="${val//\"/\`\"}"
...
is_valid_username() {
    local username="$1"
    [ "${#username}" -ge 1 ] && [ "${#username}" -le 64 ] || return 1
...
find_cert_in_bundle_by_subject() {
    local subject="$1"
    local bundle="$2"
    local tmp_dir cert cert_subject
    ...
    awk -v dir="$tmp_dir" '
        /-----BEGIN CERTIFICATE-----/ {
            in_cert = 1
            count++
            file = sprintf("%s/cert-%05d.pem", dir, count)
        }
        in_cert { print >> file }
        /-----END CERTIFICATE-----/ {
            close(file)
    ...
    for cert in "$tmp_dir"/cert-*.pem; do
        [ -f "$cert" ] || continue
        cert_subject=$(openssl x509 -in "$cert" -noout -subject -nameopt RFC2253 2>/dev/null | sed 's/^subject=//')
        if [ "$cert_subject" = "$subject" ]; then
            cat "$cert"
            rm -rf "$tmp_dir"
            return 0
        fi
...
build_letsencrypt_openvpn_ca_bundle() {
    local chain_file="${CERTS_DIR}/le_chain.crt"
    local out_file="${CERTS_DIR}/le_openvpn_ca.crt"
    local bundle_path=""
    local tmp_dir last_cert issuer subject root_cert
    ...
    awk -v dir="$tmp_dir" '
        /-----BEGIN CERTIFICATE-----/ {
            in_cert = 1
            count++
            file = sprintf("%s/chain-%05d.pem", dir, count)
        }
        in_cert { print >> file }
        /-----END CERTIFICATE-----/ {
            close(file)
    ...
    last_cert=$(ls "$tmp_dir"/chain-*.pem 2>/dev/null | sort | tail -1)
    if [ -z "$last_cert" ] || [ ! -f "$last_cert" ]; then
        rm -rf "$tmp_dir"
        return 1
    fi

    issuer=$(openssl x509 -in "$last_cert" -noout -issuer -nameopt RFC2253 2>/dev/null | sed 's/^issuer=//')
    subject=$(openssl x509 -in "$last_cert" -noout -subject -nameopt RFC2253 2>/dev/null | sed 's/^subject=//')
    ...
    if [ "$issuer" = "$subject" ]; then
        cp "$chain_file" "$out_file"
        rm -rf "$tmp_dir"
        return 0
    fi

    bundle_path=$(get_system_ca_bundle) || {
        rm -rf "$tmp_dir"
        return 1
    }

    root_cert=$(find_cert_in_bundle_by_subject "$issuer" "$bundle_path") || {
        rm -rf "$tmp_dir"
        return 1
    }
...
# Convert a CIDR prefix length to dotted-decimal netmask
cidr_to_netmask() {
    local prefix="$1"
...
ipv4_to_wg_ipv6() {
    ...
    local IFS=.
    local _o1 _o2 o3 o4
    read -r _o1 _o2 o3 o4 <<< "$ip"
    printf '%s::%x:%x' "${WG_SUBNET6%::/64}" "$o3" "$o4"
}
...
# Increment an IP address by 1 while avoiding .0/.1/.255 host octets
increment_ip() {
    local ip="$1"
    local IFS=.
...
next_wg_ip() {
    ...
    local current_int first_int pool_end_int
    current_int=$(ip_to_int "$current")
    first_int=$(ip_to_int "$WG_FIRST_CLIENT")
    pool_end_int=$(ip_to_int "$WG_POOL_END")

    if [ "$current_int" -lt "$first_int" ]; then
        return 1
    fi
    if [ "$current_int" -gt "$pool_end_int" ]; then
...
next_ovpn_ip() {
    ...
    local current_int first_int pool_end_int
    current_int=$(ip_to_int "$current")
    first_int=$(ip_to_int "$OVPN_FIRST_CLIENT")
    pool_end_int=$(ip_to_int "$OVPN_POOL_END")
...
# Read a line and abort the current flow cleanly on EOF/interrupted stdin.
safe_read() {
    if ! read "$@"; then
        print_error "Input stream closed. Aborting."
        return 1
    fi
}

# Validate selected VPNs against distro-specific package availability before
# we start mutating the system. This avoids half-installed hosts on platforms
# where a required server package is not shipped by the currently enabled repos.
validate_selected_vpns() {
    if [ "$OS_ID" = "fedora" ] && in_list "$VPN_L2TP" "$SETUP_VPNS" && cmd_exists dnf; then
        if ! dnf list --available xl2tpd 2>/dev/null | grep -q '^xl2tpd\.'; then
            print_error "Fedora's enabled repositories on this host do not provide 'xl2tpd'."
            print_error "L2TP/IPsec cannot be installed until a repository containing xl2tpd is enabled."
            print_error "Re-run the script without L2TP selected, or enable an xl2tpd-providing repository first."
            exit 1
        fi
    fi
}
...
get_primary_ipv4_subnet() {
    local iface="$1"
    ip -o -f inet addr show "$iface" 2>/dev/null | awk '!/ 127\./ {print $4; exit}'
}

get_primary_ipv6_subnet() {
    local iface="$1"
    ip -o -f inet6 addr show "$iface" scope global 2>/dev/null | awk '!/ fe80:/ {print $4; exit}'
}
#==============================================================================
# §4  OS DETECTION
#==============================================================================
...
detect_os() {
    ...
    case "$OS_ID" in
        ...
        opensuse*|sles|sled)
            ...
            PKG_INSTALL="zypper --non-interactive install --no-confirm --auto-agree-with-licenses"
            PKG_REMOVE="zypper --non-interactive remove --no-confirm"
            ;;
        alpine)
            PKG_MANAGER="apk"
        ...
        *)
            ...
            if echo "$OS_LIKE" | grep -qi "debian\|ubuntu"; then
                ...
                PKG_REMOVE="dnf remove -y -q"
                OS_ID="rhel"
            elif echo "$OS_LIKE" | grep -qi "suse\|opensuse"; then
                PKG_MANAGER="zypper"
                PKG_UPDATE="zypper --non-interactive refresh"
                PKG_INSTALL="zypper --non-interactive install --no-confirm --auto-agree-with-licenses"
                PKG_REMOVE="zypper --non-interactive remove --no-confirm"
...
save_state() {
    ...
    # Rewrite via awk instead of sed so values containing shell/sed metacharacters
    # can't corrupt the update expression during user creation or bulk changes.
    ...
    ' "$STATE_FILE" > "$tmp_file" || {
        rm -f "$tmp_file"
        print_error "Failed to save state key: ${key}"
        return 1
    }
...
delete_state() {
    ...
    awk -F= -v key="$key" '$1 != key { print }' "$STATE_FILE" > "$tmp_file" || {
        rm -f "$tmp_file"
        print_error "Failed to delete state key: ${key}"
        return 1
    }
...
delete_state_prefix() {
    ...
    awk -F= -v prefix="$prefix" 'index($1, prefix) != 1 { print }' "$STATE_FILE" > "$tmp_file" || {
        rm -f "$tmp_file"
        print_error "Failed to delete state prefix: ${prefix}"
        return 1
    }
...
load_all_state() {
    if [ -f "$STATE_FILE" ]; then
        # Load the common state values that the script actually uses.
...
reconcile_wireguard_cleanup() {
    vpn_is_installed "$VPN_WG" && return 0

    local users changed="false"
    users=$(get_users_list)

    if [ -n "$(get_state "WG_SERVER_PUBKEY")" ] || [ -n "$(get_state "WG_NEXT_IP")" ] || \
       awk -F= '$1 ~ /^WG_IP_/ { found=1; exit } END { exit(found ? 0 : 1) }' "$STATE_FILE" 2>/dev/null; then
        delete_state "WG_SERVER_PUBKEY" 2>/dev/null || true
        delete_state "WG_NEXT_IP" 2>/dev/null || true
        delete_state_prefix "WG_IP_" 2>/dev/null || true
        changed="true"
    fi
    ...
    if [ -n "$users" ]; then
        while IFS= read -r u; do
            [ -z "$u" ] && continue
            if [ -f "${CERTS_DIR}/users/${u}/wg_client.key" ] || \
               [ -f "${CERTS_DIR}/users/${u}/wg_client.pub" ] || \
               [ -f "${CERTS_DIR}/users/${u}/wg_client.psk" ] || \
               [ -f "${PROFILES_BASE}/${u}/${u}_wireguard.conf" ]; then
                rm -f "${CERTS_DIR}/users/${u}/wg_client.key" \
                      "${CERTS_DIR}/users/${u}/wg_client.pub" \
                      "${CERTS_DIR}/users/${u}/wg_client.psk" \
                      "${PROFILES_BASE}/${u}/${u}_wireguard.conf"
                generate_connection_info "$u" "" "$(get_state "L2TP_PSK")" >/dev/null 2>&1 || true
                changed="true"
            fi
        done <<< "$users"
    fi
    ...
    if [ "$changed" = "true" ]; then
        print_info "Cleaned up stale WireGuard state and artifacts from an older uninstall."
    fi
}
...
append_ikev2_cert_conn_overrides() {
    local ipsec_conf="$1"
    local username

    while IFS= read -r username; do
        [ -z "$username" ] && continue
        cat >> "$ipsec_conf" << CERT_CONN
...
restore_user_allocation_state() {
    local username="$1"
    local old_wg_next="$2"
    local old_ovpn_next="$3"

    delete_state "WG_IP_${username}" 2>/dev/null || true
    delete_state "OVPN_IP_${username}" 2>/dev/null || true

    if [ -n "$old_wg_next" ]; then
        save_state "WG_NEXT_IP" "$old_wg_next"
    else
        delete_state "WG_NEXT_IP" 2>/dev/null || true
    fi

    if [ -n "$old_ovpn_next" ]; then
        save_state "OVPN_NEXT_IP" "$old_ovpn_next"
    else
        delete_state "OVPN_NEXT_IP" 2>/dev/null || true
    fi
}
...
# On Ubuntu 24.04+, strongswan.service (swanctl) conflicts with the
# starter-based ipsec.conf flow used by this script. Keep this mutation out of
# get_strongswan_svc() so read-only status checks do not change host state.
prepare_strongswan_service() {
    local ss_svc
    ss_svc=$(get_strongswan_svc)
    if [ "$ss_svc" = "strongswan-starter" ]; then
...
# Restart strongSwan in a way that works with Ubuntu 24.04's starter-based unit.
# `ipsec restart` and naive unit restarts can leave stale pid files behind, which
# makes later starts think charon is still running when it is not.
restart_strongswan() {
    local ss_svc
    ss_svc=$(get_strongswan_svc)
    prepare_strongswan_service
    ...
    if [ "$ss_svc" = "strongswan-starter" ]; then
        systemctl stop "$ss_svc" 2>/dev/null || true
        pkill -x charon 2>/dev/null || true
        pkill -x starter 2>/dev/null || true
        rm -f /var/run/charon.pid /var/run/starter.charon.pid
        local ipsec_cmd
        ipsec_cmd=$(get_ipsec_cmd)
        systemctl start "$ss_svc" &>/dev/null || { [ -n "$ipsec_cmd" ] && "$ipsec_cmd" start &>/dev/null; } || true
    else
        systemctl restart "$ss_svc" &>/dev/null || true
    fi
    sleep 2
...
_IPSEC_CONF_PATH=""
get_ipsec_conf_path() {
    if [ -n "$_IPSEC_CONF_PATH" ]; then
        printf '%s' "$_IPSEC_CONF_PATH"
        return
    fi

    if [ -d /etc/strongswan ] || [ -f /etc/strongswan/ipsec.conf ]; then
        _IPSEC_CONF_PATH="/etc/strongswan/ipsec.conf"
    else
        _IPSEC_CONF_PATH="/etc/ipsec.conf"
    fi
...
_IPSEC_SECRETS_PATH=""
get_ipsec_secrets_path() {
    if [ -n "$_IPSEC_SECRETS_PATH" ]; then
        printf '%s' "$_IPSEC_SECRETS_PATH"
        return
    fi

    if [ -d /etc/strongswan ] || [ -f /etc/strongswan/ipsec.secrets ]; then
        _IPSEC_SECRETS_PATH="/etc/strongswan/ipsec.secrets"
    else
        _IPSEC_SECRETS_PATH="/etc/ipsec.secrets"
    fi
...
_IPSEC_DATA_DIR=""
get_ipsec_data_dir() {
    if [ -n "$_IPSEC_DATA_DIR" ]; then
        printf '%s' "$_IPSEC_DATA_DIR"
        return
    fi

    if [ -d /etc/strongswan/ipsec.d ]; then
        _IPSEC_DATA_DIR="/etc/strongswan/ipsec.d"
    else
        _IPSEC_DATA_DIR="/etc/ipsec.d"
    fi
...
_IPSEC_BIN=""
get_ipsec_cmd() {
    if [ -n "$_IPSEC_BIN" ]; then
        printf '%s' "$_IPSEC_BIN"
        return
    fi

    if command -v ipsec >/dev/null 2>&1; then
        _IPSEC_BIN=$(command -v ipsec)
    elif [ -x /usr/sbin/ipsec ]; then
        _IPSEC_BIN="/usr/sbin/ipsec"
    elif [ -x /sbin/ipsec ]; then
        _IPSEC_BIN="/sbin/ipsec"
    else
        _IPSEC_BIN=""
    fi
...
strongswan_show_status() {
    local ipsec_cmd
    ipsec_cmd=$(get_ipsec_cmd)
    if [ -n "$ipsec_cmd" ]; then
        "$ipsec_cmd" statusall 2>/dev/null
        return $?
    fi
...
strongswan_reload_config() {
    local ipsec_cmd
    ipsec_cmd=$(get_ipsec_cmd)
    if [ -n "$ipsec_cmd" ]; then
        "$ipsec_cmd" reload >/dev/null 2>&1 || "$ipsec_cmd" rereadall >/dev/null 2>&1
        return $?
    fi
    if command -v swanctl >/dev/null 2>&1; then
...
get_openvpn_conf_path() {
    local ovpn_svc
    ovpn_svc=$(get_openvpn_svc)
    if [ "$ovpn_svc" = "openvpn-server@server" ]; then
        printf '%s' "${OPENVPN_DIR}/server/server.conf"
    else
        printf '%s' "${OPENVPN_DIR}/server.conf"
    fi
}

get_openvpn_unpriv_group() {
...
# Build the rightdns= line for IKEv2, combining IPv4 and IPv6 resolvers.
build_ikev2_dns_line() {
    local dns_values=""
    local value=""

    for value in "$@"; do
        [ -z "$value" ] && continue
        dns_values=$(add_to_list "$value" "$dns_values")
    done

    [ -n "$dns_values" ] && printf 'rightdns=%s' "$dns_values"
}

# Rewrite rightdns lines inside the managed ikev2-eap and ikev2-cert blocks.
# This is more reliable than a blanket sed replacement because it also repairs
# configs where the rightdns line was deleted or corrupted.
set_ikev2_dns_lines() {
    local dns1="$1"
    local dns2="$2"
    local dns1_v6="$3"
    local dns2_v6="$4"
    local conf_file="${5:-$(get_ipsec_conf_path)}"
    [ -f "$conf_file" ] || return 1

    local dns_line=""
    dns_line=$(build_ikev2_dns_line "$dns1" "$dns2" "$dns1_v6" "$dns2_v6")

    local tmp_file
    tmp_file=$(mktemp) || return 1

    awk -v dns_line="$dns_line" '
        /^conn ikev2-eap$/  { in_block=1; print; next }
        /^conn ikev2-cert$/ { in_block=1; print; next }
        /^conn / && $0 !~ /^conn ikev2-(eap|cert)$/ { in_block=0 }
        {
            if (in_block && $0 ~ /^[[:space:]]*rightdns=/) {
                next
            }
            print
            if (in_block && $0 ~ /^[[:space:]]*rightsourceip=/ && dns_line != "") {
                print "    " dns_line
            }
        }
    ' "$conf_file" > "$tmp_file" && mv "$tmp_file" "$conf_file"
    local status=$?
    [ $status -ne 0 ] && rm -f "$tmp_file"
    return $status
}
...
save_iptables() {
    if is_debian_based; then
        ...
        if [ "$(get_state "IPV6_ENABLED")" = "yes" ] && cmd_exists ip6tables-save; then
            ip6tables-save > /etc/iptables/rules.v6 2>/dev/null || true
        fi
        cat > /etc/systemd/system/vpn-iptables-restore.service << 'IPT_RESTORE'
[Unit]
Description=Restore iptables rules managed by vpn-setup.sh
...
[Service]
Type=oneshot
ExecStart=/bin/sh -c 'if command -v iptables-restore >/dev/null 2>&1 && [ -f /etc/iptables/rules.v4 ]; then iptables-restore < /etc/iptables/rules.v4; fi; if command -v ip6tables-restore >/dev/null 2>&1 && [ -f /etc/iptables/rules.v6 ]; then ip6tables-restore < /etc/iptables/rules.v6; fi'
RemainAfterExit=yes
...
IPT_RESTORE
        ...
        systemctl enable vpn-iptables-restore.service >/dev/null 2>&1 || true
    fi
}
...
is_public_ipv4() {
    ...
    case "$ip" in
        0.*|10.*|127.*|169.254.*|192.168.*|255.255.255.255) return 1 ;;
...
ask_vpn_selection() {
    ...
    while true; do
        ...
        safe_read -r vpn_choice || return 1
...
ask_server_address() {
    ...
    while true; do
        ...
        safe_read -r addr_choice || return 1
        case "$addr_choice" in
            1)
                SETUP_ADDR_TYPE="dns"
                echo ""
                while true; do
                    ...
                    safe_read -r SETUP_ADDRESS || return 1
                    SETUP_ADDRESS=$(trim_whitespace "$SETUP_ADDRESS")
                    if [ -z "$SETUP_ADDRESS" ]; then
                        print_warning "  Hostname cannot be empty."
                    elif ! is_valid_hostname "$SETUP_ADDRESS"; then
                        print_warning "  Invalid hostname. Use a full DNS name like vpn.example.com."
                    else
                        break
                    fi
                done
                ...
                while true; do
                    ...
                    safe_read -r SETUP_LE_EMAIL || return 1
                    SETUP_LE_EMAIL=$(trim_whitespace "$SETUP_LE_EMAIL")
                    if ! has_non_whitespace_content "$SETUP_LE_EMAIL"; then
                        print_warning "  Email cannot be empty."
                    elif ! is_valid_email "$SETUP_LE_EMAIL"; then
            ...
            2)
                ...
                if [ "$detected_ip" = "<unknown>" ]; then
                    ...
                    while true; do
                        ...
                        safe_read -r SETUP_ADDRESS || return 1
                        SETUP_ADDRESS=$(trim_whitespace "$SETUP_ADDRESS")
                        if is_valid_ipv4 "$SETUP_ADDRESS"; then
                            break
                        else
...
ask_dns_servers() {
    ...
    while true; do
        ...
        safe_read -r dns_input || return 1
...
ask_user_credentials() {
    ...
    while true; do
        ...
        safe_read -r SETUP_USERNAME || return 1
        SETUP_USERNAME=$(trim_whitespace "$SETUP_USERNAME")
        if [ -z "$SETUP_USERNAME" ]; then
            print_warning "  Username cannot be empty."
        elif is_valid_username "$SETUP_USERNAME"; then
            break
        else
            print_warning "  Username must be 1-64 characters and use only letters, numbers, '.', '_', '@', '-'."
        fi
    done
    ...
    while true; do
        ...
        safe_read -rs pass1 || return 1
        echo ""
        echo -en "${YELLOW}  ?${NC}  Confirm password: "
        safe_read -rs pass2 || return 1
        echo ""
        if ! has_non_whitespace_content "$pass1"; then
    ...
    if in_list "$VPN_IKEV2" "$SETUP_VPNS" || in_list "$VPN_L2TP" "$SETUP_VPNS"; then
        ...
        while true; do
            ...
            safe_read -rs psk1 || return 1
            echo ""
            echo -en "${YELLOW}  ?${NC}  Confirm Pre-Shared Key: "
            safe_read -rs psk2 || return 1
            echo ""
            if ! has_non_whitespace_content "$psk1"; then
...
install_base_dependencies() {
    ...
    local common_pkgs="curl wget openssl ca-certificates iptables"

    if is_debian_based; then
        print_step "Installing base packages (Debian/Ubuntu)..."
        DEBIAN_FRONTEND=noninteractive eval "$PKG_INSTALL net-tools iproute2 $common_pkgs python3 python3-pip" || true
        ...
        eval "$PKG_INSTALL net-tools iproute $common_pkgs python3 python3-pip" || true
        ...
        print_step "Installing base packages (openSUSE/SLES)..."
        eval "$PKG_INSTALL net-tools iproute2 $common_pkgs python3" || true
        local py_pip_pkg=""
        py_pip_pkg=$(python3 -c 'import sys; print(f"python{sys.version_info.major}{sys.version_info.minor}-pip")' 2>/dev/null || true)
        if [ -n "$py_pip_pkg" ]; then
            eval "$PKG_INSTALL $py_pip_pkg" >/dev/null 2>&1 || true
        fi
        if service_is_active "firewalld"; then
            print_warning "firewalld is active on this system. Disabling it so iptables-managed VPN rules remain authoritative."
...
setup_firewall_base() {
    ...
    if ! iptables -t nat -C POSTROUTING -s "${IKEV2_CERT_SUBNET}" -o "${iface}" -j MASQUERADE &>/dev/null; then
        iptables -t nat -A POSTROUTING -s "${IKEV2_CERT_SUBNET}" -o "${iface}" -j MASQUERADE
    fi
    ...
    if [ "$SETUP_IPV6" = "yes" ]; then
        ...
        if ! ip6tables -t nat -C POSTROUTING -s "${IKEV2_SUBNET6}" -o "${iface}" -j MASQUERADE &>/dev/null; then
            ip6tables -t nat -A POSTROUTING -s "${IKEV2_SUBNET6}" -o "${iface}" -j MASQUERADE
        fi
        if ! ip6tables -t nat -C POSTROUTING -s "${IKEV2_CERT_SUBNET6}" -o "${iface}" -j MASQUERADE &>/dev/null; then
            ip6tables -t nat -A POSTROUTING -s "${IKEV2_CERT_SUBNET6}" -o "${iface}" -j MASQUERADE
        fi
    else
...
should_force_rsa_letsencrypt() {
    if [ -n "$SETUP_VPNS" ] && in_list "$VPN_IKEV2" "$SETUP_VPNS"; then
        return 0
    fi
    vpn_is_installed "$VPN_IKEV2"
}
...
install_certbot() {
    ...
    if is_debian_based; then
        ...
        local certbot_pkg=""
        certbot_pkg=$(python3 -c 'import sys; print(f"python{sys.version_info.major}{sys.version_info.minor}-certbot")' 2>/dev/null || true)
        if [ -n "$certbot_pkg" ]; then
            eval "$PKG_INSTALL $certbot_pkg" >/dev/null 2>&1 || true
            eval "$PKG_INSTALL certbot-systemd-timer" >/dev/null 2>&1 || true
        fi
    fi
...
obtain_letsencrypt_cert() {
    ...
    local request_rsa=false

    if ! is_valid_hostname "$domain"; then
        print_error "Invalid Let's Encrypt hostname: ${domain}"
        return 1
    fi
    if ! is_valid_email "$email"; then
        print_error "Invalid Let's Encrypt email address: ${email}"
        return 1
    fi
    ...
    if should_force_rsa_letsencrypt; then
        certbot_key_args=(--key-type rsa --rsa-key-size 4096)
        request_rsa=true
        print_info "Preferring an RSA Let's Encrypt certificate because IKEv2 is enabled."
    fi
    ...
    if [ $certbot_status -ne 0 ] && [ "$request_rsa" = true ] && grep -q -- '--key-type' /tmp/certbot_output.log 2>/dev/null; then
        print_warning "certbot rejected the RSA key-type flag; retrying without it."
        # shellcheck disable=SC2086
        certbot certonly \
            --standalone \
            --non-interactive \
    ...
    if ! build_letsencrypt_openvpn_ca_bundle; then
        print_error "Failed to build Let's Encrypt OpenVPN CA bundle."
        return 1
    fi
...
cp "${LE_DIR}/chain.pem"     "${CERTS_DIR}/le_chain.crt"
chmod 600 "${CERTS_DIR}/server.key"
if command -v awk >/dev/null 2>&1 && command -v openssl >/dev/null 2>&1; then
    bundle_path=""
    for candidate in \
        /etc/ssl/certs/ca-certificates.crt \
        /etc/pki/tls/certs/ca-bundle.crt \
    ...
    if [ -n "$bundle_path" ]; then
        ...
        if [ -n "$tmp_dir" ]; then
            ...
            ' "${CERTS_DIR}/le_chain.crt"
            last_cert=$(ls "$tmp_dir"/chain-*.pem 2>/dev/null | sort | tail -1)
            if [ -n "$last_cert" ] && [ -f "$last_cert" ]; then
                issuer=$(openssl x509 -in "$last_cert" -noout -issuer -nameopt RFC2253 2>/dev/null | sed 's/^issuer=//')
                subject=$(openssl x509 -in "$last_cert" -noout -subject -nameopt RFC2253 2>/dev/null | sed 's/^subject=//')
                if [ -n "$issuer" ] && [ "$issuer" != "$subject" ]; then
                    ...
                    if [ -n "$cert_dir" ]; then
                        ...
                        if [ -n "$found_root" ]; then
                            {
                                cat "${CERTS_DIR}/le_chain.crt"
                                printf '\n'
                                cat "$found_root"
                                printf '\n'
                            } > "${CERTS_DIR}/le_openvpn_ca.crt"
                        else
                            cp "${CERTS_DIR}/le_chain.crt" "${CERTS_DIR}/le_openvpn_ca.crt"
                        fi
                        rm -rf "$cert_dir"
                    else
                        cp "${CERTS_DIR}/le_chain.crt" "${CERTS_DIR}/le_openvpn_ca.crt"
...
# Update state file
if [ -f "$STATE_FILE" ]; then
    if grep -q '^SERVER_KEY_TYPE=' "$STATE_FILE" 2>/dev/null; then
        sed -i "s|^SERVER_KEY_TYPE=.*|SERVER_KEY_TYPE=${KEY_TYPE}|" "$STATE_FILE"
    else
        echo "SERVER_KEY_TYPE=${KEY_TYPE}" >> "$STATE_FILE"
    fi
fi

# IKEv2 (strongSwan)
if [ -d /etc/strongswan/ipsec.d ]; then
    IPSEC_DATA_DIR="/etc/strongswan/ipsec.d"
else
    IPSEC_DATA_DIR="/etc/ipsec.d"
fi

if [ -d "$IPSEC_DATA_DIR" ]; then
    cp "${CERTS_DIR}/server.crt" "${IPSEC_DATA_DIR}/certs/server.crt"
    cp "${CERTS_DIR}/server.key" "${IPSEC_DATA_DIR}/private/server.key"
    chmod 600 "${IPSEC_DATA_DIR}/private/server.key"
    # Update ipsec.secrets key type declaration
    if [ -f /etc/strongswan/ipsec.secrets ]; then
        ipsec_secrets="/etc/strongswan/ipsec.secrets"
    else
        ipsec_secrets="/etc/ipsec.secrets"
    fi
    if [ -f "$ipsec_secrets" ]; then
        sed -i "s|^: RSA server\.key|: ${KEY_TYPE} server.key|" "$ipsec_secrets"
        sed -i "s|^: ECDSA server\.key|: ${KEY_TYPE} server.key|" "$ipsec_secrets"
    fi
    if systemctl cat strongswan-starter >/dev/null 2>&1; then
        systemctl stop strongswan >/dev/null 2>&1 || true
        systemctl disable strongswan >/dev/null 2>&1 || true
        systemctl mask strongswan >/dev/null 2>&1 || true
        systemctl stop strongswan-starter >/dev/null 2>&1 || true
        pkill -x charon >/dev/null 2>&1 || true
        pkill -x starter >/dev/null 2>&1 || true
        rm -f /var/run/charon.pid /var/run/starter.charon.pid
        systemctl start strongswan-starter >/dev/null 2>&1 || ipsec start >/dev/null 2>&1 || true
    else
...
install_ikev2_packages() {
    ...
    if is_debian_based; then
        ...
        # the main strongSwan package. Try the split-package install first and
        # fall back cleanly when the plugin package does not exist.
        eval "$PKG_INSTALL strongswan strongswan-plugin-eap-mschapv2" 2>/dev/null || \
        eval "$PKG_INSTALL strongswan" || {
            print_error "Failed to install strongSwan."
            exit 1
        }
    elif is_suse_based; then
        eval "$PKG_INSTALL strongswan strongswan-plugin-eap-mschapv2" 2>/dev/null || \
    ...
    if ! cmd_exists ipsec && ! systemctl cat strongswan >/dev/null 2>&1 && ! systemctl cat strongswan-starter >/dev/null 2>&1; then
        print_error "strongSwan did not install correctly on this system."
        exit 1
    fi
}
...
install_ikev2() {
    ...
    local ipsec_data_dir
    ipsec_data_dir=$(get_ipsec_data_dir)
    mkdir -p "${ipsec_data_dir}"/{cacerts,certs,private}
    cp "${CERTS_DIR}/ca.crt"     "${ipsec_data_dir}/cacerts/"
    cp "${CERTS_DIR}/server.crt" "${ipsec_data_dir}/certs/"
    cp "${CERTS_DIR}/server.key" "${ipsec_data_dir}/private/"
    chmod 600 "${ipsec_data_dir}/private/server.key"
...
configure_ikev2() {
    ...
    ipv6_enabled=$(get_state "IPV6_ENABLED")
    local ipsec_conf
    ipsec_conf=$(get_ipsec_conf_path)
    local ipsec_secrets
    ipsec_secrets=$(get_ipsec_secrets_path)

    print_step "Writing ipsec.conf..."

    # Backup existing config
    [ -f "$ipsec_conf" ] && cp "$ipsec_conf" "${ipsec_conf}.bak.$(date +%Y%m%d%H%M%S)"

    local dns1_v6 dns2_v6
    dns1_v6=$(get_state "DNS1_IPV6")
    dns2_v6=$(get_state "DNS2_IPV6")
    ...
    local rightsourceip_cert="${IKEV2_CERT_POOL}"
    local leftsubnet_val="0.0.0.0/0"
    if [ "$ipv6_enabled" = "yes" ]; then
        rightsourceip_eap="${rightsourceip_eap},${IKEV2_SUBNET6}"
        rightsourceip_cert="${rightsourceip_cert},${IKEV2_CERT_SUBNET6}"
        leftsubnet_val="0.0.0.0/0,::/0"
    fi

    cat > "$ipsec_conf" << IPSEC_CONF
...
conn %default
    ...
    ikelifetime=1440m
    lifetime=1440m
    ike=aes256gcm16-prfsha384-ecp384,aes256gcm16-prfsha256-ecp256,aes256gcm16-prfsha256-ecp384,aes256-sha2_384-prfsha384-ecp384,aes256-sha2_256-prfsha256-ecp384,aes256-sha2_256-prfsha256-modp2048
    esp=aes256gcm16-ecp384,aes256gcm16,aes256-sha2_384-ecp384,aes256-sha2_256-ecp384,aes256-sha2_256
...
IPSEC_CONF
    ...
    append_ikev2_cert_conn_overrides "$ipsec_conf"

    print_step "Writing ipsec.secrets..."
    [ -f "$ipsec_secrets" ] && cp "$ipsec_secrets" "${ipsec_secrets}.bak.$(date +%Y%m%d%H%M%S)"
    ...
    cat > "$ipsec_secrets" << IPSEC_SECRETS
...
IPSEC_SECRETS
    ...
    if in_list "$VPN_L2TP" "$SETUP_VPNS" || vpn_is_installed "$VPN_L2TP"; then
        ...
        if [ -n "$psk" ]; then
            ...
            echo "%any %any : PSK \"${escaped_psk}\"" >> "$ipsec_secrets"
        fi
    fi
...
configure_ikev2_firewall() {
    ...
    # Cert pool subnet
    fw_add FORWARD -s "${IKEV2_CERT_SUBNET}" -j ACCEPT
    fw_add FORWARD -d "${IKEV2_CERT_SUBNET}" -j ACCEPT
    # IPv6
    fw6_add INPUT -p udp --dport "${IKEV2_PORT}" -j ACCEPT
    fw6_add INPUT -p udp --dport "${IKEV2_NAT_PORT}" -j ACCEPT
    if [ "$(get_state "IPV6_ENABLED")" = "yes" ] || [ "$SETUP_IPV6" = "yes" ]; then
        fw6_add FORWARD -s "${IKEV2_SUBNET6}" -j ACCEPT
        fw6_add FORWARD -d "${IKEV2_SUBNET6}" -j ACCEPT
        fw6_add FORWARD -s "${IKEV2_CERT_SUBNET6}" -j ACCEPT
        fw6_add FORWARD -d "${IKEV2_CERT_SUBNET6}" -j ACCEPT
    fi

    # Clamp MSS for TCP traffic exiting the tunnel to avoid PMTU issues on Apple NAT-T clients.
    if [ -n "$iface" ]; then
        if ! iptables -t mangle -C FORWARD --match policy --pol ipsec --dir in -s "${IKEV2_SUBNET}" -o "$iface" -p tcp -m tcp --tcp-flags SYN,RST SYN -m tcpmss --mss "${IKEV2_TCPMSS_MIN}:${IKEV2_TCPMSS_MAX}" -j TCPMSS --set-mss "${IKEV2_TCPMSS_VALUE}" &>/dev/null; then
            iptables -t mangle -A FORWARD --match policy --pol ipsec --dir in -s "${IKEV2_SUBNET}" -o "$iface" -p tcp -m tcp --tcp-flags SYN,RST SYN -m tcpmss --mss "${IKEV2_TCPMSS_MIN}:${IKEV2_TCPMSS_MAX}" -j TCPMSS --set-mss "${IKEV2_TCPMSS_VALUE}"
        fi
        if ! iptables -t mangle -C FORWARD --match policy --pol ipsec --dir in -s "${IKEV2_CERT_SUBNET}" -o "$iface" -p tcp -m tcp --tcp-flags SYN,RST SYN -m tcpmss --mss "${IKEV2_TCPMSS_MIN}:${IKEV2_TCPMSS_MAX}" -j TCPMSS --set-mss "${IKEV2_TCPMSS_VALUE}" &>/dev/null; then
            iptables -t mangle -A FORWARD --match policy --pol ipsec --dir in -s "${IKEV2_CERT_SUBNET}" -o "$iface" -p tcp -m tcp --tcp-flags SYN,RST SYN -m tcpmss --mss "${IKEV2_TCPMSS_MIN}:${IKEV2_TCPMSS_MAX}" -j TCPMSS --set-mss "${IKEV2_TCPMSS_VALUE}"
        fi
    fi
    save_iptables
...
add_ikev2_user() {
    ...
    sed -i "/^%any ${sed_user} : EAP /d" "$ipsec_secrets"

    echo "%any ${escaped_user} : EAP \"${escaped_pass}\"" >> "$ipsec_secrets"
    ...
    if [ -f "$user_crt" ]; then
        local ipsec_data_dir
        ipsec_data_dir=$(get_ipsec_data_dir)
        cp "$user_crt" "${ipsec_data_dir}/certs/${username}_client.crt"
        cp "$user_key" "${ipsec_data_dir}/private/${username}_client.key"
        chmod 600 "${ipsec_data_dir}/private/${username}_client.key"
    fi
    ...
    sed -i "/^# BEGIN ikev2-cert-${sed_uname}$/,/^# END ikev2-cert-${sed_uname}$/d" "$ipsec_conf"
    cat >> "$ipsec_conf" << CERT_CONN
...
CERT_CONN
    ...
    if [ "$skip_restart" != "true" ]; then
        restart_strongswan >/dev/null 2>&1 || true
    fi
...
remove_ikev2_user() {
    ...
    escaped_user=$(escape_ipsec "$username")
    sed_user=$(sed_escape_pattern "$escaped_user")
    sed -i "/^${sed_user} : EAP /d" "$ipsec_secrets"
    sed -i "/^%any ${sed_user} : EAP /d" "$ipsec_secrets"

    # Remove certs
    local ipsec_data_dir
    ipsec_data_dir=$(get_ipsec_data_dir)
    rm -f "${ipsec_data_dir}/certs/${username}_client.crt"
    rm -f "${ipsec_data_dir}/private/${username}_client.key"
...
install_l2tp_packages() {
    ...
    # Fedora's default enabled repos may not ship xl2tpd on newer releases.
    # Detect this before any install attempt so we fail with a clear message
    # instead of leaving the host partially configured mid-run.
    if [ "$OS_ID" = "fedora" ] && cmd_exists dnf; then
        if ! dnf list --available xl2tpd 2>/dev/null | grep -q '^xl2tpd\.'; then
            print_error "The enabled Fedora repositories on this host do not provide 'xl2tpd'."
            print_error "L2TP/IPsec cannot be installed until a repo containing xl2tpd is enabled."
            print_error "Either enable a repository that ships xl2tpd, or re-run without L2TP selected."
            exit 1
        fi
    fi
...
install_l2tp() {
    ...
    if [ -z "$(get_ipsec_cmd)" ] && ! systemctl cat strongswan >/dev/null 2>&1 && ! systemctl cat strongswan-starter >/dev/null 2>&1; then
        install_ikev2_packages
    fi
...
configure_l2tp_ipsec() {
    ...
    if grep -q "conn L2TP-PSK" "$ipsec_conf" 2>/dev/null; then
        return 0
    fi

    cat >> "$ipsec_conf" << L2TP_CONN
...
L2TP_CONN
    ...
    local escaped_psk
    escaped_psk=$(escape_ipsec "$psk")
    sed -i "/^%any %any : PSK /d" "$ipsec_secrets"
    echo "%any %any : PSK \"${escaped_psk}\"" >> "$ipsec_secrets"
...
configure_wireguard() {
    ...
    local wg_prefix
    wg_prefix=$(ipv4_cidr_prefix "$WG_SUBNET")
    local wg_address="${WG_SERVER_IP}/${wg_prefix}"
    local dns_push="${dns1}"
    ...
    if [ "$ipv6_enabled" = "yes" ]; then
        wg_address="${WG_SERVER_IP}/${wg_prefix}, ${WG_SERVER_IP6}/64"
        postup_rules="${postup_rules}; ip6tables -A FORWARD -i %i -j ACCEPT; ip6tables -A FORWARD -o %i -j ACCEPT; ip6tables -t nat -A POSTROUTING -s ${WG_SUBNET6} -o ${iface} -j MASQUERADE"
        postdown_rules="${postdown_rules}; ip6tables -D FORWARD -i %i -j ACCEPT; ip6tables -D FORWARD -o %i -j ACCEPT; ip6tables -t nat -D POSTROUTING -s ${WG_SUBNET6} -o ${iface} -j MASQUERADE"
    fi
...
add_wireguard_user() {
    ...
    local client_allowed_ips="${client_ip}/32"
    if [ "$ipv6_enabled" = "yes" ]; then
        client_allowed_ips="${client_allowed_ips}, $(ipv4_to_wg_ipv6 "$client_ip")/128"
    fi
...
WG_PEER
    ...
    if wg show wg0 &>/dev/null; then
        if wg addconf wg0 <(wg-quick strip wg0) &>/dev/null; then
            ...
            wg set wg0 peer "$client_pubkey" preshared-key <(echo "$client_psk") allowed-ips "${client_allowed_ips}" &>/dev/null || true
        else
...
remove_wireguard_user() {
    ...
    if [ -n "$client_pubkey" ]; then
        ...
        local wg_tmp
        wg_tmp=$(mktemp /tmp/wg0_tmp.XXXXXX.conf 2>/dev/null || mktemp) || wg_tmp=""
        python3 - "$client_pubkey" /etc/wireguard/wg0.conf << 'PYEOF' 2>/dev/null || \
        { [ -n "$wg_tmp" ] && awk -v uname="${username}" '$0 == "# User: " uname{p=1;next} p && /^[[:space:]]*$/{p=0;next} !p{print}' /etc/wireguard/wg0.conf > "$wg_tmp" && mv "$wg_tmp" /etc/wireguard/wg0.conf; } || true
import sys, re
...
install_openvpn_packages() {
    ...
    elif is_rhel_based || is_suse_based; then
...
configure_openvpn() {
    ...
    local ovpn_conf
    ovpn_conf=$(get_openvpn_conf_path)
...
push \"dhcp-option DNS ${dns2}\""
    ...
    local ovpn_netmask
    ovpn_netmask=$(cidr_to_netmask "$(ipv4_cidr_prefix "$OVPN_SUBNET")")
    local server_directive="server ${ovpn_network} ${ovpn_netmask}"
...
OVPN_CONF
    ...
    sed -i "s/^group nogroup$/group $(get_openvpn_unpriv_group)/" "${ovpn_conf}"
...
setup_openvpn_auth_script() {
    ...
    local ovpn_group
    ovpn_group=$(get_openvpn_unpriv_group)
...
exit 1
VERIFY_SCRIPT
    chown "root:${ovpn_group}" "${OPENVPN_DIR}/auth/verify.sh"
...
add_openvpn_user() {
    ...
    local ovpn_conf
    ovpn_conf=$(get_openvpn_conf_path)
    if ! grep -q "^client-config-dir" "${ovpn_conf}" 2>/dev/null; then
        echo "client-config-dir ${OPENVPN_DIR}/ccd" >> "${ovpn_conf}"
    fi
...
create_vpn_user() {
    ...
    local added_ikev2="false"
    local added_l2tp="false"
    local added_wg="false"
    local added_ovpn="false"
    local old_wg_next old_ovpn_next
    old_wg_next=$(get_state "WG_NEXT_IP")
    old_ovpn_next=$(get_state "OVPN_NEXT_IP")
    ...
    if vpn_is_installed "$VPN_IKEV2"; then
        add_ikev2_user "$username" "$password" "$skip_restart" || {
            print_error "Failed to add ${username} to IKEv2."
            restore_user_allocation_state "$username" "$old_wg_next" "$old_ovpn_next"
            rm -rf "${CERTS_DIR}/users/${username}"
            return 1
        }
        added_ikev2="true"
    fi
    ...
    if vpn_is_installed "$VPN_L2TP"; then
        add_l2tp_user "$username" "$password" "$skip_restart" || {
            print_error "Failed to add ${username} to L2TP. Rolling back partial user creation."
            [ "$added_ikev2" = "true" ] && remove_ikev2_user "$username" "$skip_restart"
            restore_user_allocation_state "$username" "$old_wg_next" "$old_ovpn_next"
            rm -rf "${CERTS_DIR}/users/${username}"
            return 1
        }
    ...
    if vpn_is_installed "$VPN_WG"; then
        add_wireguard_user "$username" || {
            print_error "Failed to add ${username} to WireGuard. Rolling back partial user creation."
            [ "$added_l2tp" = "true" ] && remove_l2tp_user "$username"
            [ "$added_ikev2" = "true" ] && remove_ikev2_user "$username" "$skip_restart"
            restore_user_allocation_state "$username" "$old_wg_next" "$old_ovpn_next"
            rm -rf "${CERTS_DIR}/users/${username}"
            return 1
    ...
    if vpn_is_installed "$VPN_OVPN"; then
        add_openvpn_user "$username" "$password" "$skip_restart" || {
            print_error "Failed to add ${username} to OpenVPN. Rolling back partial user creation."
            [ "$added_wg" = "true" ] && remove_wireguard_user "$username"
            [ "$added_l2tp" = "true" ] && remove_l2tp_user "$username"
            [ "$added_ikev2" = "true" ] && remove_ikev2_user "$username" "$skip_restart"
            restore_user_allocation_state "$username" "$old_wg_next" "$old_ovpn_next"
            rm -rf "${CERTS_DIR}/users/${username}"
    ...
    generate_all_profiles "$username" "$password" "$psk" || {
        print_error "Failed to generate profile files for ${username}."
        [ "$added_ovpn" = "true" ] && remove_openvpn_user "$username"
        [ "$added_wg" = "true" ] && remove_wireguard_user "$username"
        [ "$added_l2tp" = "true" ] && remove_l2tp_user "$username"
        [ "$added_ikev2" = "true" ] && remove_ikev2_user "$username" "$skip_restart"
        restore_user_allocation_state "$username" "$old_wg_next" "$old_ovpn_next"
        rm -rf "${PROFILES_BASE}/${username}" "${CERTS_DIR}/users/${username}"
...
generate_ikev2_eap_mobileconfig() {
    ...
    local trust_hints=""
    if [ "$addr_type" = "dns" ]; then
        trust_hints="                <key>ServerCertificateCommonName</key>
...
generate_ikev2_cert_mobileconfig() {
    ...
    # XML-escape the P12 password — it's embedded in a <string> element
    local xml_password
...
generate_ikev2_sswan() {
    ...
    # JSON-escape password — it's embedded in a double-quoted JSON value
    local json_password
...
generate_ikev2_windows_ps1() {
    ...
    # PowerShell-escape the P12 password — it's embedded in a double-quoted string
    local ps_password
    ps_password=$(ps_escape "$password")
...
if (Test-Path \$p12Path) {
    \$p12Password = ConvertTo-SecureString "${ps_password}" -AsPlainText -Force
...
generate_wireguard_conf() {
    ...
    local allowed_ips
    allowed_ips=$(get_state "WG_ALLOWED_IPS")
    if [ -z "$allowed_ips" ]; then
        allowed_ips="0.0.0.0/0"
        [ "$ipv6_enabled" = "yes" ] && allowed_ips="${allowed_ips}, ::/0"
    fi
    if [ "$ipv6_enabled" = "yes" ]; then
        client_address="${client_ip}/32, $(ipv4_to_wg_ipv6 "$client_ip")/128"
        [ -n "$dns1_v6" ] && dns_entry="${dns_entry}, ${dns1_v6}"
        [ -n "$dns2_v6" ] && [ "$dns2_v6" != "$dns1_v6" ] && dns_entry="${dns_entry}, ${dns2_v6}"
    fi
...
generate_openvpn_ovpn() {
    ...
    # In LE mode, use a trust bundle that includes the issuing LE chain plus the root CA.
    # OpenVPN uses <ca> as trust anchors, so the intermediate chain alone is not sufficient.
    # In self-signed mode, use our own CA cert.
    if is_letsencrypt; then
        ca_crt_content=$(cat "${CERTS_DIR}/le_openvpn_ca.crt")
    else
...
show_management_menu() {
    while true; do
        ...
        echo ""
        echo -en "${YELLOW}  ?${NC}  Enter choice: "
        safe_read -r menu_choice || return 1
...
add_user_menu() {
    ...
    while true; do
        ...
        safe_read -r new_username || return
        new_username=$(trim_whitespace "$new_username")
        [ "$new_username" = "0" ] && return
        if [ -z "$new_username" ]; then
            print_warning "Username cannot be empty."
        elif ! is_valid_username "$new_username"; then
            print_warning "Username must be 1-64 characters and use only letters, digits, '.', '_', '@', '-'."
    ...
    while true; do
        echo -en "${YELLOW}  ?${NC}  Password (hidden): "
        safe_read -rs new_password || return
        echo ""
        echo -en "${YELLOW}  ?${NC}  Confirm password: "
        safe_read -rs confirm_pass || return
        echo ""
        if ! has_non_whitespace_content "$new_password"; then
            print_warning "Password cannot be empty."
    ...
    if vpn_is_installed "$VPN_IKEV2" || vpn_is_installed "$VPN_L2TP"; then
        while true; do
            ...
            safe_read -rs new_psk || return
            echo ""
            echo -en "${YELLOW}  ?${NC}  Confirm PSK: "
            safe_read -rs confirm_psk || return
            echo ""
            if ! has_non_whitespace_content "$new_psk"; then
                print_warning "PSK cannot be empty."
...
batch_add_from_csv() {
    ...
    while IFS= read -r line || [ -n "$line" ]; do
        ...
        csv_user=$(trim_whitespace "$csv_user")

        local row_ok=true

        if [ -z "$csv_user" ] || ! is_valid_username "$csv_user"; then
        ...
        if ! has_non_whitespace_content "$csv_pass"; then
            print_warning "Line ${line_num}: password is empty"
            row_ok=false
        fi

        if $needs_psk && ! has_non_whitespace_content "$csv_psk"; then
...
update_single_user() {
    ...
    while true; do
        ...
        if [ -z "$new_password" ]; then
            break
        elif ! has_non_whitespace_content "$new_password"; then
            print_warning "Password cannot be only whitespace."
            new_password=""
            continue
        fi
    ...
    while true; do
        ...
        if [ -z "$new_psk" ]; then
            break
        elif ! has_non_whitespace_content "$new_psk"; then
            print_warning "PSK cannot be only whitespace."
            new_psk=""
            continue
        fi
...
update_user_credentials() {
    ...
    if [ -n "$new_password" ] && ! has_non_whitespace_content "$new_password"; then
        print_error "Refusing to set a password that is empty or whitespace-only."
        return 1
    fi
    if [ -n "$new_psk" ] && ! has_non_whitespace_content "$new_psk"; then
        print_error "Refusing to set a PSK that is empty or whitespace-only."
        return 1
    fi
    ...
    if [ -n "$new_password" ]; then
        ...
        if vpn_is_installed "$VPN_IKEV2" && [ -f "$ipsec_secrets" ]; then
            local escaped_user escaped_pass sed_user
            escaped_user=$(escape_ipsec "$username")
        ...
        generate_all_profiles "$username" "$new_password" "$effective_psk" || {
            print_error "Failed to regenerate profile files for ${username}. Password change was applied to backend credentials only."
            return 1
        }
    ...
    if [ -n "$new_psk" ]; then
        ...
        if { vpn_is_installed "$VPN_IKEV2" || vpn_is_installed "$VPN_L2TP"; } && [ -f "$ipsec_secrets" ]; then
            local escaped_psk
...
batch_update_from_csv() {
    ...
    while IFS= read -r line || [ -n "$line" ]; do
        ...
        if [ -z "$csv_user" ]; then
            ...
            row_ok=false
        elif ! is_valid_username "$csv_user"; then
            print_warning "Line ${line_num}: invalid username '${csv_user}'"
            row_ok=false
        elif ! in_list "$csv_user" "$(get_state "USERS_LIST")"; then
        ...
        if ! has_non_whitespace_content "$csv_pass" && ! has_non_whitespace_content "$csv_psk"; then
            print_warning "Line ${line_num}: both password and PSK are empty — nothing to update"
            row_ok=false
        elif [ -n "$csv_pass" ] && ! has_non_whitespace_content "$csv_pass"; then
            print_warning "Line ${line_num}: password cannot be whitespace-only"
            row_ok=false
        elif [ -n "$csv_psk" ] && ! has_non_whitespace_content "$csv_psk"; then
            print_warning "Line ${line_num}: PSK cannot be whitespace-only"
            row_ok=false
        fi
...
change_server_address_menu() {
    ...
    local current_type
    current_type=$(get_state "ADDRESS_TYPE")
        ...
        case "$choice" in
            ...
            1)
                ...
                while true; do
                    ...
                    safe_read -r new_addr || return
                    new_addr=$(trim_whitespace "$new_addr")
                    if [ -z "$new_addr" ]; then
                        print_warning "Hostname cannot be empty."
                    elif ! is_valid_hostname "$new_addr"; then
                        print_warning "Invalid hostname. Use a full DNS name like vpn.example.com."
                    else
                        break
                    fi
    ...
    if [ "$new_type" = "dns" ]; then
        ...
        if [ -n "$existing_email" ]; then
            ...
            safe_read -r le_email || return
            le_email=$(trim_whitespace "$le_email")
            [ -z "$le_email" ] && le_email="$existing_email"
            if ! is_valid_email "$le_email"; then
                print_warning "Invalid email address."
                press_enter
                return
            fi
        else
            while true; do
                ...
                safe_read -r le_email || return
                le_email=$(trim_whitespace "$le_email")
                if ! has_non_whitespace_content "$le_email"; then
                    print_warning "  Email cannot be empty."
                elif ! is_valid_email "$le_email"; then
                    print_warning "  Please enter a valid email address."
                else
                    break
                fi
    ...
    if [ "$new_type" = "dns" ]; then
        if obtain_letsencrypt_cert "$new_addr" "$le_email"; then
            ...
            if ask_yn "Continue with a self-signed certificate instead?" "y"; then
                ...
                save_state "ADDRESS_TYPE" "$current_type"
                press_enter
                return
            fi
        fi
    ...
    local ipsec_data_dir
    ipsec_data_dir=$(get_ipsec_data_dir)
    cp "${CERTS_DIR}/server.crt" "${ipsec_data_dir}/certs/" 2>/dev/null || true
    cp "${CERTS_DIR}/server.key" "${ipsec_data_dir}/private/" 2>/dev/null || true
    chmod 600 "${ipsec_data_dir}/private/server.key" 2>/dev/null || true
    ...
    if vpn_is_installed "$VPN_IKEV2" || vpn_is_installed "$VPN_L2TP"; then
        ...
        local ipsec_conf ipsec_secrets
        ipsec_conf=$(get_ipsec_conf_path)
        ipsec_secrets=$(get_ipsec_secrets_path)
        sed -i "s|leftid=.*|leftid=${escaped_leftid}|" "$ipsec_conf" 2>/dev/null || true
        ...
        sed -i "s|^: RSA server\.key|: ${new_key_type} server.key|" "$ipsec_secrets" 2>/dev/null || true
        sed -i "s|^: ECDSA server\.key|: ${new_key_type} server.key|" "$ipsec_secrets" 2>/dev/null || true
        restart_strongswan >/dev/null 2>&1 || true
    fi
...
change_dns_menu() {
    ...
    local ovpn_conf
    ovpn_conf=$(get_openvpn_conf_path)
    if vpn_is_installed "$VPN_OVPN" && [ -f "${ovpn_conf}" ]; then
        sed -i '/^push "dhcp-option DNS /d' "${ovpn_conf}"
        if [ -n "$SETUP_DNS1" ]; then
            echo "push \"dhcp-option DNS ${SETUP_DNS1}\"" >> "${ovpn_conf}"
        fi
    ...
    local ipsec_conf
    ipsec_conf=$(get_ipsec_conf_path)
    if vpn_is_installed "$VPN_IKEV2" && [ -f "$ipsec_conf" ]; then
        if set_ikev2_dns_lines "$SETUP_DNS1" "$SETUP_DNS2" "$SETUP_DNS1_V6" "$SETUP_DNS2_V6" "$ipsec_conf"; then
            restart_strongswan >/dev/null 2>&1 || true
        else
            print_warning "Failed to rewrite IKEv2 DNS lines in ${ipsec_conf}"
        fi
    fi
...
uninstall_ikev2() {
    ...
    if [ -f "$ipsec_conf" ]; then
        sed -i '/^conn ikev2-eap$/,/^$/d' "$ipsec_conf"
        sed -i '/^conn ikev2-cert$/,/^$/d' "$ipsec_conf"
        sed -i '/^# BEGIN ikev2-cert-/,/^# END ikev2-cert-/d' "$ipsec_conf"
    fi
    ...
    if ! vpn_is_installed "$VPN_L2TP"; then
        ...
        rm -f "$ipsec_conf" "$ipsec_secrets"
        rm -rf "$(get_ipsec_data_dir)"
    else
        # L2TP still needs strongSwan — just remove EAP entries from ipsec.secrets
        sed -i '/: EAP /d' "$ipsec_secrets"
        restart_strongswan >/dev/null 2>&1 || true
    fi
    ...
    fw_delete FORWARD -s "${IKEV2_SUBNET}" -j ACCEPT 2>/dev/null || true
    fw_delete FORWARD -d "${IKEV2_SUBNET}" -j ACCEPT 2>/dev/null || true
    fw_delete FORWARD -s "${IKEV2_CERT_SUBNET}" -j ACCEPT 2>/dev/null || true
    fw_delete FORWARD -d "${IKEV2_CERT_SUBNET}" -j ACCEPT 2>/dev/null || true
    ...
    fw6_delete FORWARD -s "${IKEV2_SUBNET6}" -j ACCEPT 2>/dev/null || true
    fw6_delete FORWARD -d "${IKEV2_SUBNET6}" -j ACCEPT 2>/dev/null || true
    fw6_delete FORWARD -s "${IKEV2_CERT_SUBNET6}" -j ACCEPT 2>/dev/null || true
    fw6_delete FORWARD -d "${IKEV2_CERT_SUBNET6}" -j ACCEPT 2>/dev/null || true
    if [ -n "$iface" ]; then
        iptables -t mangle -D FORWARD --match policy --pol ipsec --dir in -s "${IKEV2_SUBNET}" -o "$iface" -p tcp -m tcp --tcp-flags SYN,RST SYN -m tcpmss --mss "${IKEV2_TCPMSS_MIN}:${IKEV2_TCPMSS_MAX}" -j TCPMSS --set-mss "${IKEV2_TCPMSS_VALUE}" 2>/dev/null || true
        iptables -t mangle -D FORWARD --match policy --pol ipsec --dir in -s "${IKEV2_CERT_SUBNET}" -o "$iface" -p tcp -m tcp --tcp-flags SYN,RST SYN -m tcpmss --mss "${IKEV2_TCPMSS_MIN}:${IKEV2_TCPMSS_MAX}" -j TCPMSS --set-mss "${IKEV2_TCPMSS_VALUE}" 2>/dev/null || true
    fi
    save_iptables
    ...
    if [ -n "$users" ]; then
        while IFS= read -r u; do
            [ -z "$u" ] && continue
            rm -f \
                "${PROFILES_BASE}/${u}/${u}_ikev2_eap.mobileconfig" \
                "${PROFILES_BASE}/${u}/${u}_ikev2_cert.mobileconfig" \
                "${PROFILES_BASE}/${u}/${u}_ikev2.sswan" \
                "${PROFILES_BASE}/${u}/${u}_ikev2_cert.sswan" \
                "${PROFILES_BASE}/${u}/${u}_ikev2_windows.ps1"
            if ! vpn_is_installed "$VPN_OVPN"; then
                rm -f \
                    "${PROFILES_BASE}/${u}/${u}_ca.crt" \
                    "${PROFILES_BASE}/${u}/${u}_client_cert.p12"
            fi
            generate_connection_info "$u" "" "$(get_state "L2TP_PSK")" >/dev/null 2>&1 || true
        done <<< "$users"
    fi
...
uninstall_l2tp() {
    ...
    if [ -f "$ipsec_conf" ]; then
        sed -i '/^# L2TP\/IPsec/,/^$/d' "$ipsec_conf" 2>/dev/null || true
        sed -i '/^conn L2TP-PSK/,/^$/d' "$ipsec_conf" 2>/dev/null || true
    fi
    if [ -f "$ipsec_secrets" ]; then
        sed -i '/^%any %any : PSK /d' "$ipsec_secrets"
    fi
    ...
    local saved_psk
    saved_psk=$(get_state "L2TP_PSK")
    if ! vpn_is_installed "$VPN_IKEV2"; then
        delete_state "L2TP_PSK" 2>/dev/null || true
    fi
    if [ -n "$users" ]; then
        while IFS= read -r u; do
            [ -z "$u" ] && continue
            generate_connection_info "$u" "" "$saved_psk" >/dev/null 2>&1 || true
        done <<< "$users"
    fi
    ...
    if [ -n "$users" ]; then
        while IFS= read -r u; do
            ...
            rm -f "${CERTS_DIR}/users/${u}/wg_client.key" \
                  "${CERTS_DIR}/users/${u}/wg_client.pub" \
                  "${CERTS_DIR}/users/${u}/wg_client.psk"
            rm -f "${PROFILES_BASE}/${u}/${u}_wireguard.conf"
            generate_connection_info "$u" "" "$(get_state "L2TP_PSK")" >/dev/null 2>&1 || true
        done <<< "$users"
    fi
    save_iptables
...
uninstall_openvpn() {
    ...
    fw_delete FORWARD -i tun0 -j ACCEPT 2>/dev/null || true
    fw_delete FORWARD -o tun0 -j ACCEPT 2>/dev/null || true
    fw6_delete INPUT -p udp --dport "${OVPN_PORT}" -j ACCEPT 2>/dev/null || true
    fw6_delete FORWARD -i tun0 -j ACCEPT 2>/dev/null || true
    fw6_delete FORWARD -o tun0 -j ACCEPT 2>/dev/null || true
    iptables -t nat -D POSTROUTING -s "${OVPN_SUBNET}" -o "${iface}" -j MASQUERADE 2>/dev/null || true
    ip6tables -t nat -D POSTROUTING -s "${OVPN_SUBNET6}" -o "${iface}" -j MASQUERADE 2>/dev/null || true

    delete_state "OPENVPN_SERVICE" 2>/dev/null || true
    delete_state "OVPN_NEXT_IP" 2>/dev/null || true
    delete_state_prefix "OVPN_IP_" 2>/dev/null || true
    mark_vpn_uninstalled "$VPN_OVPN"

    if [ -n "$users" ]; then
        while IFS= read -r u; do
            [ -z "$u" ] && continue
            rm -f "${PROFILES_BASE}/${u}/${u}_openvpn.ovpn"
            generate_connection_info "$u" "" "$(get_state "L2TP_PSK")" >/dev/null 2>&1 || true
        done <<< "$users"
    fi
...
renew_letsencrypt_menu() {
    ...
    if certbot renew --cert-name vpn-server --force-renewal 2>&1; then
        print_success "Renewal complete. Deploy hook has restarted VPN services."
    else
        print_error "Let's Encrypt renewal failed. Check the output above for the root cause."
        print_warning "Common causes: port 80 blocked, DNS mismatch, or another service bound to port 80."
    fi
    press_enter
}
    ...
    case "$st_choice" in
        ...
        3)
            ...
            while true; do
                ...
                safe_read -r custom_subnets || return
                custom_subnets=$(trim_whitespace "$custom_subnets")
                if [ -z "$custom_subnets" ]; then
                    print_warning "Subnet list cannot be empty."
                elif ! is_valid_cidr_list "$custom_subnets"; then
                    print_warning "Please enter valid IPv4/IPv6 CIDR subnets separated by commas."
                else
...
apply_split_tunneling() {
    ...
    if vpn_is_installed "$VPN_WG"; then
        ...
        if [ "$mode" = "full" ]; then
            ...
            [ "$ipv6_enabled" = "yes" ] && wg_allowed_ips="${wg_allowed_ips}, ::/0"
        else
            wg_allowed_ips=$(echo "$subnets" | awk -F',' '{
                for (i = 1; i <= NF; i++) {
                    gsub(/^[[:space:]]+|[[:space:]]+$/, "", $i)
                    if ($i != "") {
                        out = out (out ? ", " : "") $i
        ...
        save_state "WG_ALLOWED_IPS" "$wg_allowed_ips"
    ...
    if vpn_is_installed "$VPN_OVPN" && [ -f "${ovpn_conf}" ]; then
        ...
        sed -i '/^push "redirect-gateway/d' "${ovpn_conf}"
        sed -i '/^push "route /d' "${ovpn_conf}"
        sed -i '/^push "route-ipv6 /d' "${ovpn_conf}"

        if [ "$mode" = "full" ]; then
            echo 'push "redirect-gateway def1 bypass-dhcp"' >> "${ovpn_conf}"
            if [ "$ipv6_enabled" = "yes" ]; then
                echo 'push "route-ipv6 2000::/3"' >> "${ovpn_conf}"
            fi
        else
            ...
            echo "$subnets" | tr ',' '\n' | while read -r subnet; do
                ...
                [ -z "$subnet" ] && continue
                if [[ "$subnet" == *:* ]]; then
                    echo "push \"route-ipv6 ${subnet}\"" >> "${ovpn_conf}"
                else
                    local net mask
                    net=$(echo "$subnet" | cut -d/ -f1)
                    mask=$(cidr_to_mask "$(echo "$subnet" | cut -d/ -f2)")
                    echo "push \"route ${net} ${mask}\"" >> "${ovpn_conf}"
                fi
            done
        fi
        service_restart "$(get_openvpn_svc)"
    ...
    if vpn_is_installed "$VPN_IKEV2" && [ -f "$ipsec_conf" ]; then
        if [ "$mode" = "full" ]; then
            local leftsubnet_full="0.0.0.0/0"
            [ "$ipv6_enabled" = "yes" ] && leftsubnet_full="${leftsubnet_full},::/0"
            sed -i "s|leftsubnet=.*|leftsubnet=${leftsubnet_full}|" "$ipsec_conf"
        else
            local ike_subnets escaped_subnets
            ike_subnets=$(echo "$subnets" | awk -F',' '{
                for (i = 1; i <= NF; i++) {
                    gsub(/^[[:space:]]+|[[:space:]]+$/, "", $i)
                    if ($i != "") {
                        out = out (out ? "," : "") $i
            ...
            sed -i "s|leftsubnet=.*|leftsubnet=${escaped_subnets}|" "$ipsec_conf"
        fi
        restart_strongswan >/dev/null 2>&1 || true
...
server_subnet_access_menu() {
    ...
    local iface server_subnet4 server_subnet6
    iface=$(get_primary_iface)
    server_subnet4=$(get_primary_ipv4_subnet "$iface")
    server_subnet6=$(get_primary_ipv6_subnet "$iface")

    echo -e "  Detected IPv4 subnet: ${BOLD}${server_subnet4:-unknown}${NC}"
    if [ "$(get_state "IPV6_ENABLED")" = "yes" ]; then
        echo -e "  Detected IPv6 subnet: ${BOLD}${server_subnet6:-unknown}${NC}"
    fi
    echo ""

    local current current_targets
    current=$(get_state "SERVER_SUBNET_ACCESS")
    current_targets=$(get_state "SERVER_SUBNET_ACCESS_TARGETS")
    if [ "$current" = "enabled" ] && [ -n "$current_targets" ]; then
        echo -e "  Current state: ${BOLD}enabled (${current_targets})${NC}"
    else
    ...
    echo ""
    echo -en "${YELLOW}  ?${NC}  Enter choice: "
    safe_read -r sub_choice || return

    case "$sub_choice" in
        1)
            local target_subnets default_subnets
            default_subnets="$server_subnet4"
            if [ -n "$server_subnet6" ] && [ "$(get_state "IPV6_ENABLED")" = "yes" ]; then
                default_subnets="${default_subnets:+${default_subnets},}${server_subnet6}"
            fi
            if [ -n "$default_subnets" ]; then
                while true; do
                    echo -en "${YELLOW}  ?${NC}  Server subnet(s) [${default_subnets}]: "
                    safe_read -r target_subnets || return
                    target_subnets="${target_subnets:-$default_subnets}"
                    target_subnets=$(trim_whitespace "$target_subnets")
                    if is_valid_cidr_list "$target_subnets"; then
                        break
                    fi
                    print_warning "Please enter valid IPv4/IPv6 CIDR subnets separated by commas."
                done
                ...
                while true; do
                    echo -en "${YELLOW}  ?${NC}  Enter server subnet(s) (comma-separated, e.g. 192.168.1.0/24,2001:db8::/64): "
                    safe_read -r target_subnets || return
                    target_subnets=$(trim_whitespace "$target_subnets")
                    if ! has_non_whitespace_content "$target_subnets"; then
                        print_warning "Subnet list cannot be empty."
                    elif ! is_valid_cidr_list "$target_subnets"; then
                        print_warning "Please enter valid IPv4/IPv6 CIDR subnets separated by commas."
                    else
                        break
                    fi
            ...
            echo "$target_subnets" | tr ',' '\n' | while IFS= read -r target_subnet; do
                target_subnet=$(echo "$target_subnet" | xargs)
                [ -z "$target_subnet" ] && continue

                if [[ "$target_subnet" == *:* ]]; then
                    for vpn_subnet in "${WG_SUBNET6}" "${OVPN_SUBNET6}"; do
                        fw6_add FORWARD -s "$vpn_subnet" -d "$target_subnet" -j ACCEPT
                        fw6_add FORWARD -s "$target_subnet" -d "$vpn_subnet" -j ACCEPT
                    done
                else
                    for vpn_subnet in "${IKEV2_SUBNET}" "${IKEV2_CERT_SUBNET}" "${L2TP_SUBNET}" "${WG_SUBNET}" "${OVPN_SUBNET}"; do
            ...
            save_iptables
            save_state "SERVER_SUBNET_ACCESS" "enabled"
            save_state "SERVER_SUBNET_ACCESS_TARGETS" "$target_subnets"
            print_success "VPN clients can now access: ${target_subnets}"
            ;;
        2)
            local saved_subnet
            saved_subnet=$(get_state "SERVER_SUBNET_ACCESS_TARGETS")
            if [ -n "$saved_subnet" ]; then
                echo "$saved_subnet" | tr ',' '\n' | while IFS= read -r target_subnet; do
                    target_subnet=$(echo "$target_subnet" | xargs)
                    [ -z "$target_subnet" ] && continue

                    if [[ "$target_subnet" == *:* ]]; then
                        for vpn_subnet in "${WG_SUBNET6}" "${OVPN_SUBNET6}"; do
                            fw6_delete FORWARD -s "$vpn_subnet" -d "$target_subnet" -j ACCEPT 2>/dev/null || true
                            fw6_delete FORWARD -s "$target_subnet" -d "$vpn_subnet" -j ACCEPT 2>/dev/null || true
                        done
                    else
                        for vpn_subnet in "${IKEV2_SUBNET}" "${IKEV2_CERT_SUBNET}" "${L2TP_SUBNET}" "${WG_SUBNET}" "${OVPN_SUBNET}"; do
                            fw_delete FORWARD -s "$vpn_subnet" -d "$target_subnet" -j ACCEPT 2>/dev/null || true
                            fw_delete FORWARD -s "$target_subnet" -d "$vpn_subnet" -j ACCEPT 2>/dev/null || true
                        done
                    fi
                done
                save_iptables
            ...
            delete_state "SERVER_SUBNET_ACCESS_TARGETS" 2>/dev/null || true
...
client_subnet_access_menu() {
    ...
    case "$csa_choice" in
        1)
            ...
            fw_add FORWARD -i "$iface" -d "${IKEV2_CERT_SUBNET}" -j ACCEPT
            ...
            if [ "$(get_state "IPV6_ENABLED")" = "yes" ]; then
                fw6_add FORWARD -i "$iface" -d "${WG_SUBNET6}" -j ACCEPT
                fw6_add FORWARD -i "$iface" -d "${OVPN_SUBNET6}" -j ACCEPT
            fi
        ...
        2)
            ...
            fw_delete FORWARD -i "$iface" -d "${IKEV2_CERT_SUBNET}" -j ACCEPT 2>/dev/null || true
            ...
            fw6_delete FORWARD -i "$iface" -d "${WG_SUBNET6}" -j ACCEPT 2>/dev/null || true
            fw6_delete FORWARD -i "$iface" -d "${OVPN_SUBNET6}" -j ACCEPT 2>/dev/null || true
            save_iptables
...
add_port_forward_rule() {
    ...
    while true; do
        ...
        safe_read -r proto || return
        proto=$(trim_whitespace "$proto")
        case "${proto,,}" in
            tcp|udp) break ;;
    ...
    while true; do
        ...
        safe_read -r ext_port || return
        ext_port=$(trim_whitespace "$ext_port")
    ...
    local client_ip
    while true; do
    echo -en "${YELLOW}  ?${NC}  VPN client IP (e.g. 172.22.64.2): "
        safe_read -r client_ip || return
        client_ip=$(trim_whitespace "$client_ip")
        is_valid_ipv4 "$client_ip" && break
        print_warning "Invalid IP address."
    done
    ...
    while true; do
        ...
        safe_read -r int_port || return
        int_port=$(trim_whitespace "$int_port")
...
disable_ipv6_menu() {
    ...
    if [ "$current" = "yes" ]; then
        ...
        echo ""
        echo -en "${YELLOW}  ?${NC}  Enter choice: "
        safe_read -r ipv6_choice || return
...
IPV6_SYSCTL
    ...
    ipsec_conf=$(get_ipsec_conf_path)
    if vpn_is_installed "$VPN_IKEV2" && [ -f "$ipsec_conf" ]; then
        local tmp_ipsec_conf
        tmp_ipsec_conf=$(mktemp) || return 1
        awk '
            /^conn ikev2-eap$/  { in_block=1; print; next }
            /^conn ikev2-cert$/ { in_block=1; print; next }
            /^conn / && $0 !~ /^conn ikev2-(eap|cert)$/ { in_block=0 }
            {
                if (in_block && $0 ~ /^[[:space:]]*leftsubnet=/) {
                    sub(/,::\/0/, "", $0)
                    gsub(/,[[:space:]]*[^,]*:[^,]*/, "", $0)
                }
                if (in_block && $0 ~ /^[[:space:]]*rightsourceip=/) {
                    gsub(/,[[:space:]]*[^,]*:[^,]*/, "", $0)
                }
                if (in_block && $0 ~ /^[[:space:]]*rightdns=/) {
                    gsub(/,[[:space:]]*[^,]*:[^,]*/, "", $0)
        ...
        ' "$ipsec_conf" > "$tmp_ipsec_conf" && mv "$tmp_ipsec_conf" "$ipsec_conf"
        local awk_status=$?
        [ $awk_status -ne 0 ] && rm -f "$tmp_ipsec_conf"
        restart_strongswan >/dev/null 2>&1 || true
    fi
    ...
    if vpn_is_installed "$VPN_WG" && [ -f /etc/wireguard/wg0.conf ]; then
        local wg_prefix
        wg_prefix=$(ipv4_cidr_prefix "$WG_SUBNET")
        sed -i 's|Address = .*,.*|Address = '"${WG_SERVER_IP}/${wg_prefix}"'|' /etc/wireguard/wg0.conf
    ...
    ovpn_conf=$(get_openvpn_conf_path)
    if vpn_is_installed "$VPN_OVPN" && [ -f "${ovpn_conf}" ]; then
        sed -i '/server-ipv6/d' "${ovpn_conf}"
        sed -i '/route-ipv6/d' "${ovpn_conf}"
        service_restart "$(get_openvpn_svc)"
    fi
...
strongswan_is_active() {
    ...
    # Starter-based deployments are healthy when the long-lived charon daemon
    # is present. Ubuntu 24.04 may report strongswan-starter as inactive after
    # it launches charon, while some RHEL-family builds do not expose the
    # `ipsec` wrapper in PATH even though the daemon is healthy.
    if [ "$ss_svc" = "strongswan-starter" ]; then
        pgrep -x charon &>/dev/null || return 1
        return 0
    fi
...
validate_ikev2() {
    ...
    local ipsec_status
    ipsec_status=$(strongswan_show_status 2>/dev/null)
    if echo "$ipsec_status" | grep -q "ikev2-eap"; then
        print_done "IKEv2 EAP connection loaded"
    else
        # Try reloading connections
        print_warning "IKEv2 EAP connection not loaded — attempting strongSwan config reload..."
        strongswan_reload_config >/dev/null 2>&1
        sleep 1
        ipsec_status=$(strongswan_show_status 2>/dev/null)
        if echo "$ipsec_status" | grep -q "ikev2-eap"; then
    ...
    if echo "$ipsec_status" | grep -q "ikev2-cert"; then
        ...
        # Reload was already attempted above — just re-check with fresh status
        ipsec_status=$(strongswan_show_status 2>/dev/null)
        if echo "$ipsec_status" | grep -q "ikev2-cert"; then
            print_done "IKEv2 cert connection loaded"
        else
    ...
    if [ "$cert_type" = "letsencrypt" ]; then
        ...
        local le_lineage="${LE_CERTS_BASE}/vpn-server"
        if [ -n "$le_domain" ] && [ -f "${le_lineage}/fullchain.pem" ]; then
            if openssl x509 -in "${le_lineage}/cert.pem" -noout -checkend 0 &>/dev/null; then
...
validate_l2tp() {
    ...
    if echo "$ipsec_status" | grep -q "L2TP-PSK"; then
        ...
        print_warning "L2TP-PSK connection not loaded — attempting strongSwan config reload..."
        strongswan_reload_config >/dev/null 2>&1
        sleep 1
        ipsec_status=$(strongswan_show_status 2>/dev/null)
        if echo "$ipsec_status" | grep -q "L2TP-PSK"; then
    ...
    local ipsec_secrets
    ipsec_secrets=$(get_ipsec_secrets_path)
    if grep -q "^%any %any : PSK " "$ipsec_secrets" 2>/dev/null; then
        ...
        if [ -n "$saved_psk" ]; then
            ...
            echo "%any %any : PSK \"${escaped_psk}\"" >> "$ipsec_secrets"
            restart_strongswan >/dev/null 2>&1
            sleep 1
            if grep -q "^%any %any : PSK " "$ipsec_secrets" 2>/dev/null; then
...
validate_openvpn() {
    ...
    if [ -f "${ovpn_conf}" ]; then
        ...
        print_error "OpenVPN server config not found at ${ovpn_conf}"
        errors=$((errors + 1))
...
first_run_setup() {
    ...
    ask_vpn_selection || return 1
    ask_server_address || return 1
    ask_ipv6_support || return 1
    ask_dns_servers || return 1
    ask_user_credentials || return 1
    validate_selected_vpns
    ...
    create_vpn_user "$SETUP_USERNAME" "$SETUP_PASSWORD" "$SETUP_PSK" || {
        print_error "Failed to create the first VPN user. Aborting setup before the success summary."
        return 1
    }
...
main() {
    ...
    # Clean up stale WireGuard leftovers from older versions before deciding
    # whether this is a re-run or a fresh install. Older uninstalls could leave
    # WG_* state behind even when INSTALLED_VPNS is now empty.
    if [ -f "$STATE_FILE" ]; then
        reconcile_wireguard_cleanup
    fi
    ...
    if has_prior_install; then
        ...
        if [ -z "$saved_addr" ] || { ! is_valid_ipv4 "$saved_addr" && ! is_valid_hostname "$saved_addr"; }; then
