...
| Debian post-install validation says IKEv2 / L2TP connections are `NOT loaded in strongSwan` even though UDP `500/4500/1701` are listening | Debian ended up on the `swanctl`/`charon-systemd` stack while the script wrote `ipsec.conf`, so charon listened but did not load `ikev2-eap`, `ikev2-cert`, or `L2TP-PSK` | Fixed — Debian now explicitly installs `strongswan-starter` and no longer pulls in the swanctl/charon-systemd stack during the IKEv2 package step |
...
| Debian install stops at WireGuard with `Could not get lock /var/lib/dpkg/lock-frontend` or `wireguard-dkms has no installation candidate` | Another apt job still held the dpkg lock, then the old fallback assumed `wireguard-dkms` exists on every Debian image | Fixed — the script now waits for apt/dpkg locks to clear and retries with `wireguard-tools` before attempting `wireguard-dkms` only when that package actually exists |
